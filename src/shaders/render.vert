...
void main(void) {
    ...
    vec3 texture1 = texture2D(uTextureMap1, aTextureCoord).xyz;
    vec3 texture2 = texture2D(uTextureMap2, aTextureCoord).xyz;
    ...
    float threshold = 0.;
    if(onAddTexture == 1.){
        tColor = tIndex == 0. ? texture1 : texture2;
    ...
    color = mix(color, tColor, threshold);
